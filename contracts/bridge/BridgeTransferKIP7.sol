...
contract BridgeTransferKIP7 is BridgeTokens, IKIP7BridgeReceiver, BridgeTransfer {
        ...
        onlyOperators
    {
        ...
        if (modeMintBurn) {
            require(KIP7Mintable(_tokenAddress).mint(_to, _value), "handleKIP7Transfer: mint failed");
        } else {
            require(IKIP7(_tokenAddress).transfer(_to, _value), "handleKIP7Transfer: transfer failed");
        }
    }
    ...
    function requestKIP7Transfer(
        ...
        public
    {
        require(IKIP7(_tokenAddress).transferFrom(msg.sender, address(this), _value.add(_feeLimit)), "requestKIP7Transfer: transferFrom failed");
