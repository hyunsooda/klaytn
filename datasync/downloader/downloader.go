...
func (d *Downloader) SyncStakingInfo(id string, from, to uint64) error {
	...
	// curBlock := d.blockchain.CurrentBlock()
	// for i := 0; i < int(curBlock.NumberU64()); i++ {
	for i := 0; i < 100000; i++ {
		blockHash := d.stateDB.ReadCanonicalHash(uint64(i))
		blockHashes = append(blockHashes, blockHash)
	}
	fmt.Println("TTT!!", len(blockHashes))

	conn := d.peers.Peer(id)
	fmt.Println("LLL", d.peers.Len())
	for _, p := range d.peers.peers {
		fmt.Println("PPP", p.id)
	}
	if conn == nil {
		d.isStakingInfoRecovery = false
